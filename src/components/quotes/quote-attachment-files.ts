 ...
 *
 * ── 🔴 이름과 색조는 A/S 와 같고, 곁말 둘만 다르다 ───────────────────────
 * 딱지 이름 셋과 색조(`info` · `neutral` · `warning`)는 저쪽 그대로다 — 두 사이트의
 * 같은 줄이 다른 딱지를 달면 사람이 같은 견적서를 다른 것으로 본다. 🔴 **곁말(title)
 * 둘만 이 사이트의 사실을 말한다**:
 *
 *  · **「엑셀 전용」** — 저쪽 문장은 「[견적서 받기]가 붙인 엑셀을 내려줍니다」인데 이
 *    사이트에서는 그것이 **거짓**이다(내줄 파일이 없어 통로가 거절한다). 그래서 받기
 *    통로와 목록의 단추가 쓰는 **그 한 문장**을 그대로 쓴다
 *    (domain/quote-excel-only-download.ts) — 화면 셋과 서버가 같은 말을 한다.
 *  · **「엑셀 없음」** — 저쪽 문장은 「견적서 **수정 화면에서 붙여 주세요**」로 끝나는데
 *    이 사이트에는 그 칸이 없어 **거짓**이다. 붙일 수 있는 곳(A/S)을 가리킨다.
 *
 * 🔴 **붙이는 칸(조각 3d-3 · 3d-4)이 오면 둘 다 저쪽 문장으로 돌아간다.**
 * 「결재 PDF」 곁말은 사실을 말할 뿐이라 저쪽 그대로 쓴다.
 * ============================================================================
 */
