...
describe("작업 비용 기본 권한 — A/S 와 같은 답", () => {
  ...
  test("모르는 영역은 닫는다 — 여는 쪽으로 실패하지 않는다", () => {
    // 🔴 `domesticOrders` 는 조각 2 에서, `quotes` 는 조각 3a 에서 **아는 영역이
    // 되었다** — 그 대조는 각각 domestic-order-permission.test.ts ·
    // quote-permission.test.ts 가 한다. 여기 남은 것은 이쪽으로 올 일이 없는
    // 영역(A/S 의 사용자 관리)이다.
    assert.equal(baselinePermissionLevel("users", "SUPER_ADMIN"), "NONE");
    assert.equal(baselinePermissionLevel("repairCases", "SUPER_ADMIN"), "NONE");
  });
});
...
describe("영역 목록", () => {
  test("🔴 저장 열쇠가 A/S 가 쓰는 글자 그대로다", () => {
    ...
    // 🔴 차례도 함께 못 박는다 — A/S 의 같은 파일 차례 그대로다.
    assert.deepEqual([...PERMISSION_LEAF_KEYS], ["domesticOrders", "quotes", "repairLabor"]);
  });
  ...
  test("🔴 모르는 키는 걸러진다 — 같은 표에 A/S 의 영역 열넷이 함께 들어 있다", () => {
    ...
    // 🔴 조각 3a 에서 아는 영역이 되었다(견적서 목록·휴지통).
    assert.equal(isPermissionLeafKey("quotes"), true);
