 ...
 * 3a 가 「견적서」를 옮겨 왔다. 영역은 **그 화면이 오는 조각에서 그 화면과 함께**
 * 더한다. 미리 적어 두면 아무도 못 들어가는 메뉴가 생기거나, 더 나쁘게는 화면이
 * 없는데 권한만 열린 상태가 된다.
 *
 * ⚠️ 차례는 A/S 의 같은 파일 차례 그대로다(내자 정리 · 견적서 · 작업 비용). 아래
 * PERMISSION_LEAF_KEYS 가 이 차례를 그대로 쓰고, 시험이 그 목록을 못 박는다.
 *
...
export const PERMISSION_AREAS: readonly PermissionArea[] = [
  ...
  {
    key: "quotes",
    // 🔴 열쇠(`quotes`)·이름표·설명 모두 A/S 와 **글자 하나까지 같다**
    // (저쪽 permission-areas.ts). 설명이 갈리면 저쪽 [역할별 접근 권한] 화면이
    // 말하는 「관리」와 여기서 실제로 열리는 것이 어긋나 보인다.
    label: "견적서",
    // 만들기·고치기는 영업까지고(WRITE), 지우고 되살리는 것은 관리자 이상이다
    // (MANAGE — quote-authorization.ts). 쓰기와 갈리는 조작이 실제로 있으므로
    // 상한이 관리다. 🔴 여기를 쓰기로 내리면 휴지통이 누구에게도 열리지 않는다.
