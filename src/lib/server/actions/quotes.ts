 ...
 *
 * 🔴 **인수번호 찾기**(lookupIntakeForQuoteAction)가 들어왔다 — 조각 3b-3 이다.
 * 그 하나가 **저장하지 않는 유일한 액션**이라 문턱도 여기서만 READ 다
 * (아래 resolveReadingUser). 🔴 그 값을 참고 목록으로 늘어놓고 부품 줄로 담는 일과
 * 부품 고르개는 **액션을 더 만들지 않았다** — 참고 목록은 이 액션이 이미 돌려주는
 * 값으로 그리고, 고르개의 두 목록은 **페이지가 서버에서 읽어 넘긴다**
 * (견적서 라우트 둘의 page.tsx · queries/inventory.ts). 이 파일은 늘지 않았다.
 *
 * ── 관문은 하나다 ───────────────────────────────────────────────────────
